...
int VTrack::create_derived_objs(int flash)
{
	...
	camera_autos = new BezierAutos(mwindow, 
									this, 
									WHITE, 
									0, 
									0, 
	...
	projector_autos = new BezierAutos(mwindow, 
									this,
									WHITE,
									0,
									0,
...
{
	float samples_f = (float)samples / mwindow->sample_rate * mwindow->frame_rate;
	samples = (long)(samples_f < 0 ? samples_f - 0.5 : samples_f + 0.5);
	return samples;
}


long VTrack::units_to_samples(long &units)
{
	units = (long)((float)units / mwindow->frame_rate * mwindow->sample_rate);
	return units;
}
...
// only used for pasting automation
	if(htal->tag.title_is("CAMERAAUTOS"))
	{
		camera_autos->paste(start, end, total_length, htal, "/CAMERAAUTOS", 1, shift_autos);
	}
	else
	if(htal->tag.title_is("PROJECTORAUTOS"))
...
int VTrack::direct_copy_possible(long current_frame)
{
	int result = 1;
