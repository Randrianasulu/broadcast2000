...
RecordEngine::RecordEngine(MainWindow *mwindow, 
			Record *record, 
			File *file, 
			Asset *asset, 
			RecordLabels *labels)
...
{
	monitor_thread = new RecordThread(record, this);
	monitor_thread->initialize();
	
	record_thread = new RecordThread(record, this);
	record_thread->initialize();
...
// put at end of file
	...
	if(record->do_video) current_position = tosamples((float)(file->get_video_length(record->get_framerate())), record->get_samplerate(), record->get_framerate());
...
int RecordEngine::run_script(FileHTAL *script)
{
	...
	while(!result && !script_result)
	{
		...
		if(!result)
		{
			if(script->tag.title_is("set_mode"))
			{
				...
				gui->rec_mode_menu->update(string);
			}
			else
...
// OSS >= 3.9 crashes if playback starts before recording
		...
		record_thread->start_recording(file);
...
// start the duplex engine if necessary
		record_thread->start_recording(file, 1);
		is_saving = 1;
	}
	return 0;
return 0;
...
int RecordEngine::goto_prev_label()
{
	if(!is_saving)
	{
		...
		if(new_position != -1)
		{
			...
			if(record->do_video) file->set_video_position(toframes(new_position, record->get_samplerate(), record->get_framerate()), record->get_framerate());
...
int RecordEngine::get_vu_format() { return record->get_vu_format(); return 0;
}
int RecordEngine::get_duplex_status() { return record->duplex_status; return 0;
}
int RecordEngine::set_duplex_status(int duplex_status) { record->duplex_status = duplex_status; return 0;
}
int RecordEngine::get_dither() { return record->dither * asset->bits; return 0;
}
int RecordEngine::get_input_channels() { return asset->channels; return 0;
}
...
int RecordEngine::get_samplerate() { return asset->rate; return 0;
}
int RecordEngine::get_bits() { return asset->bits; return 0;
}
int RecordEngine::get_time_format() { return record->get_time_format(); return 0;
}
float RecordEngine::get_frame_rate() { return record->get_frame_rate(); }
int RecordEngine::get_loop_hr() { return record->loop_duration / asset->rate / 3600; return 0;
}
int RecordEngine::get_loop_min() { return record->loop_duration / asset->rate / 60 - (long)get_loop_hr() * 60; return 0;
}
int RecordEngine::get_loop_sec() { return record->loop_duration / asset->rate - (long)get_loop_hr() * 3600 - (long)get_loop_min() * 60; return 0;
}
long RecordEngine::get_loop_duration() { return record->loop_duration; }
float RecordEngine::get_min_db() { return record->get_min_db(); }
int RecordEngine::get_meter_over_hold(int divisions) { return divisions * 15; return 0;
}
int RecordEngine::get_meter_peak_hold(int divisions) { return divisions * 2; return 0;
}
int RecordEngine::get_meter_speed() { return record->get_meter_speed(); return 0;
}
...
{
	if(!record->monitor_video && value && !record->video_window_open)
	{
		gui->monitor_video_window->window->show_window();
	}
	if(gui->monitor_video_toggle && gui->monitor_video_toggle->get_value() != value) 
		gui->monitor_video_toggle->update(value);
	record->monitor_video = value;
	if(value) record->video_window_open = 1;
return 0;
}

int RecordEngine::set_monitor_audio(int value)
{
	record->monitor_audio = value;
	if(gui->monitor_audio_toggle && gui->monitor_audio_toggle->get_value() != value) 
		gui->monitor_audio_toggle->update(value);
return 0;
}
