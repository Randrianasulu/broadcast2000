...
static gint8  dv_reorder_nommx[2][64] = {
  { 0 },
  {
...
static gint8  dv_reorder_mmx[2][64] = {
  { 0 },
  {
    0, 32, 1, 33, 8, 40, 2, 34,		9, 41, 16, 48, 24, 56, 17, 49,
    10, 42, 3, 35, 4, 36, 11, 43,		18, 50, 25, 57, 26, 58, 19, 51,
...
// Only execute once in a program.
	for(i = 0; i < 64; i++)
	{
    	dv_reorder_nommx[DV_DCT_88][i] = ((dv_88_reorder_prime[i] / 8) * 8) + (dv_88_reorder_prime[i] % 8);
    	dv_reorder_mmx[DV_DCT_88][i] = ((dv_88_reorder_prime[i] % 8) * 8) + (dv_88_reorder_prime[i] / 8);
	} // for 

	for(i = 0; i < 64; i++)
	{
#if ZERO_MULT_ZIGZAG
    	dv_reorder_nommx[DV_DCT_88][i] = (dv_reorder_nommx[DV_DCT_88][i]) * sizeof(dv_coeff_t);
    	dv_reorder_nommx[DV_DCT_248][i] = (dv_reorder_nommx[DV_DCT_248][i]) * sizeof(dv_coeff_t);

    	dv_reorder_mmx[DV_DCT_88][i] = (dv_reorder_mmx[DV_DCT_88][i]) * sizeof(dv_coeff_t);
    	dv_reorder_mmx[DV_DCT_248][i] = (dv_reorder_mmx[DV_DCT_248][i]) * sizeof(dv_coeff_t);
#endif
	} // for
} // dv_parse_init
...
__inline__ void dv_parse_ac_coeffs_pass0(bitstream_t *bs,
  ...
  vlc_trace("\nB%d",b);
  // Main coeffient parsing loop
  bzero(&bl->coeffs[1], sizeof(bl->coeffs)-sizeof(bl->coeffs[0]));
  while(1) {
 ...
 *
 *  */
gint dv_parse_video_segment(dv_t *dv, dv_videosegment_t *seg) {
  gint             m, b;
  gint             mb_start;
  gint             dc;
  dv_macroblock_t *mb;
...
// Dumps debugging info to terminal
...
// DC coefficient (twos complement)
    		...
    		bl->reorder = &dv_reorder_mmx[bl->dct_mode][1];
...
// DC coefficient (twos complement)
    		...
    		bl->offset= mb_start + dv_parse_bit_start[b];
    		bl->end= mb_start + dv_parse_bit_end[b];
    		bl->reorder = &dv_reorder_nommx[bl->dct_mode][1];
    		bl->reorder_sentinel = bl->reorder + 63;
    		dv_parse_ac_coeffs_pass0(bs,mb,bl);
    		bitstream_seek_set(bs,bl->end);
    	} // for b
