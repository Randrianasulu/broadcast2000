...
// Return pixel offset from top left corner
size_t dv_place_411_macroblock(dv_macroblock_t *mb) 
{
  ...
  result = mb_row * 8 * 720 +  // block is 8 pels high, row is 720 pels wide
    mb_col * 8;       // macroblock is 4 blocks wide, 8 pels wide per block
  return(result);
} // dv_place_411_macroblock

// Return pixel offset from top left corner
size_t dv_place_420_macroblock(dv_macroblock_t *mb) 
{
  size_t result; // offset in bytes from start of image buffer
  gint mb_num; // mb number withing the 6 x 5 zig-zag pattern 
  ...
  result = mb_row * 16 * 720 + // macro block is 16 pels high, row is 720 pels wide
    mb_col * 16;       // macroblock is 2 blocks wide, 8 pels wide per block
  return(result);
