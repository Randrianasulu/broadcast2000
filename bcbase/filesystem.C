...
int FileSystem::test_filter(FileItem *file)
{
    ...
    for(j = strlen(file->name) - 1, k = strlen(filter) - 1;
	    j > 0 && k > 0 && file->name[j] == filter[k]; j--, k--)
	{
      	;
	}
    if(k == 0 || filter[0] == 0) return 1;
	return 0;
return 0;
}
...
int FileSystem::update(char *new_dir)
{
	...
	struct stat ostat;
	struct tm *mod_time;
	int i, j, k, filter_this, result;
	FileItem *new_file;
	char full_path[1024], name_only[1024];
...
// file not hidden
  		if((show_all_files && strcmp(new_filename->d_name, ".")) ||
			!strcmp(new_filename->d_name, "..") || new_filename->d_name[0] != '.')
    	{
...
// test filter
				...
				if(filter_this)
				{
      				if(!test_filter(new_file)) result = 1;
				}

// test want_directory
				if(want_directory && !new_file->is_dir) result = 1;
			}
			else
				result = 1;
...
// 			sprintf(string, "%s%s", pw->pw_dir, &new_dir[1]); 
			strcpy(new_dir, string);
			return 0;
		}
		else
		{                // another user's home directory
			char string[1024], new_user[1024];
			struct passwd *pw;
			int i, j;
