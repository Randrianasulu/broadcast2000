...
public:
	FileItem();
	FileItem(char *path, char *name, int is_dir, longest size, int month, int day, int year);
	~FileItem();
...
// Whether or not the file passes the current filter.
	...
	char filter[1024];     // what filenames have to end in to get displayed
	int want_directory;
