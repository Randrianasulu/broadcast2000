 ...
 *
 * $Id: tonal.c,v 1.1.1.1 2000/10/27 18:33:15 heroines Exp $
 *
 * $Log: tonal.c,v $
 * Revision 1.1.1.1  2000/10/27 18:33:15  heroines
 *
 *
 *
 *
...
int *tone, *noise;
{
 ...
 while(i != LAST && i < HAN_SIZE){            /* reduction of spectral lines        */
    if(power[i].x < ltg[power[i].map].hear){
       power[i].type = FALSE;
       power[i].x = DBMIN;
...
frame_params *fr_ps;
{
 ...
 for(k=0;k<stereo;k++){  /* check pcm input for 3 blocks of 384 samples */
    for(i=0;i<1152;i++) fft_buf[k][(i+off[k])%1408]= (double)buffer[k][i];
...
frame_params *fr_ps;
{
 ...
 for(k=0;k<stereo;k++){    /* check PCM input for a block of */
    for(i=0;i<384;i++)     /* 384 samples for a 512-pt. FFT  */
       fft_buf[k][(i+off[k])%640]= (double) buffer[k][i];
    for(i=0;i<FFT_SIZE/2;i++)
