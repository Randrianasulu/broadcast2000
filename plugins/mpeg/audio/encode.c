 ...
 *
 * $Id: encode.c,v 1.1.1.1 2000/10/27 18:33:13 heroines Exp $
 *
 * $Log: encode.c,v $
 * Revision 1.1.1.1  2000/10/27 18:33:13  heroines
 *
 *
 *
 *
...
unsigned long get_audio(int shmid,
				int inputsemid,
				int outputsemid,
				float **ring_buffer,
				long *ring_buffer_allocated,
...
// Fill ring buffer
	while(*ring_buffer_size < 1152 && !*end_of_file)
	{
// Wait for input
		sop.sem_num = 0;
		sop.sem_op = -1;
		sop.sem_flg = 0;
		semop(inputsemid, &sop, 1);
		...
		{
			samples = *(unsigned long*)data;
			data += sizeof(unsigned long);
			new_size = *ring_buffer_size + samples;

// Allocate ring buffer
			if(new_size > *ring_buffer_allocated)
			{
				float *new_buffer;

				new_buffer = malloc(sizeof(float) * new_size * stereo);

				for(i = 0; i < *ring_buffer_size * stereo; i++)
				{
					new_buffer[i] = (*ring_buffer)[i];
				}

				free(*ring_buffer);
				*ring_buffer = new_buffer;
				*ring_buffer_allocated = new_size;
			}
		}
...
// Free up buffer
		...
		sop.sem_op = 1;
		sop.sem_flg = 0;
		semop(outputsemid, &sop, 1);
	}

	samples_read = (*ring_buffer_size < 1152) ? *ring_buffer_size : 1152;
	...
	if(samples_read)
	{
		for(i = 0; i < samples_read; i++)
		{
			for(j = 0; j < stereo; j++)
			{
				buffer[j][i] = (*ring_buffer)[i * stereo + j];
			}
			if(j < 2) buffer[1][i] = 0;
		}
...
int k;
{
    ...
    for (i=0;i<32;i++) (*x)[k][31-i+off[k]] = (double) *(*buffer)++;
