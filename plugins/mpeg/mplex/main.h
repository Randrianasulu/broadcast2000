...
{   unsigned char  buf [SYS_HEADER_SIZE*2]; /*FIXME*/
    unsigned int size;
} Sys_header_struc;
...
#define FOR_EACH_STR(stream, streams, if_any, if_video, if_audio) \
...
#define FIND_STR(stream, streams, expresion) \
	for (stream = streams; stream && !(expresion); stream = stream->next)
#define FIND_AUDIO_STR(stream, streams, expresion) \
	FIND_STR (stream, streams, stream->type==STREAMS_AUDIO && ( expresion ))
#define FIND_VIDEO_STR(stream, streams, expresion) \
	FIND_STR (stream, streams, stream->type==STREAMS_VIDEO && ( expresion ))
...
void get_info             ();	/* Info Access Units Any Stream	*/
