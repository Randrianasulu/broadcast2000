...
void check_files (argv, audio_streams, video_streams)
char* argv[];
stream_element **audio_streams;
stream_element **video_streams;

{
    Bit_stream_struc bs;
    stream_element *stream;
    stream_element **video_next=video_streams;
    stream_element **audio_next=audio_streams;
    char n_audio_streams=0;
    char n_video_streams=0;

    do {

    stream = (stream_element *)malloc (sizeof(stream_element));
		
    stream->file_name = *argv;

    if (open_file(stream->file_name,&stream->bytes))
	exit (1);

    open_bit_stream_r (&bs, stream->file_name, BUFFER_SIZE);
    ...
    if (look_ahead (&bs, 12) == 0xfff)
    {
	printf ("File %s is a 11172-3 Audio stream.\n", stream->file_name);
	stream->next = NULL;
	*audio_next = stream;
	audio_next = &stream->next;
	stream->type = STREAMS_AUDIO;
	stream->nstream = n_audio_streams++;
    }
    else if (look_ahead (&bs, 32) == 0x1b3)
    {
	printf ("File %s is a 11172-2 Video stream.\n", stream->file_name);
	stream->next = NULL;
	*video_next = stream;
	video_next = &stream->next;
	stream->type = STREAMS_VIDEO;
	stream->nstream = n_video_streams++;
    }
    else 
    {
    ...
    *video_next = *audio_streams;

 }
void get_info (stream, startup_delay)
...

{
	switch (stream->type){
	    case STREAMS_AUDIO:
		    get_info_audio (stream->file_name, stream->units_name,
				    &stream->info.audio, startup_delay,
				    stream->bytes);
		    break;
	    case STREAMS_VIDEO:
		    get_info_video (stream->file_name, stream->units_name,
				    &stream->info.video, startup_delay,
				    stream->bytes);
		    break;
	}
}
...
printf("\n+------------------ VIDEO STREAM INFORMATION -----------------+\n");
    ...
    else if (video_info->picture_rate <16)
