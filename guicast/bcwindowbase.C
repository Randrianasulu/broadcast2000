...
// Create the window
		...
		win = XCreateWindow(display, rootwin, x, y, w, h, 0, top_level->default_depth, InputOutput, vis, mask, &attr);
		...
		size_hints.width = w;
		size_hints.height = h;
		size_hints.min_width = allow_resize ? minw : w;
		size_hints.max_width = allow_resize ? 32767 : w; 
		size_hints.min_height = allow_resize ? minh : h;
		size_hints.max_height = allow_resize ? 32767 : h; 
	...
	if(window_type == POPUP_WINDOW)
	{
		...
		win = XCreateWindow(top_level->display, top_level->rootwin, x, y, w, h, 0, top_level->default_depth, InputOutput, top_level->vis, mask, &attr);
	}
	...
	if(window_type == SUB_WINDOW)
	{
		...
		win = XCreateWindow(top_level->display, parent_window->win, x, y, w, h, 0, top_level->default_depth, InputOutput, top_level->vis, mask, &attr);
		init_window_shape();
...
void BC_WindowBase::init_cursors()
{
	...
	hseparate_cursor = XCreateFontCursor(display, XC_ul_angle);
}
...
int BC_WindowBase::add_menubar(BC_MenuBar *menu_bar)
{
...
int BC_WindowBase::add_subwindow(BC_WindowBase *subwindow)
{
...
int BC_WindowBase::add_tool(BC_WindowBase *subwindow)
{
...
int BC_WindowBase::reposition_window(int x, int y, int w, int h)
{
	...
	XMoveResizeWindow(top_level->display, win, x, y, get_w(), get_h());

	if(resize)
	{
		XFreePixmap(top_level->display, pixmap);
 		pixmap = XCreatePixmap(top_level->display, win, get_w(), get_h(), top_level->default_depth);
...
{
	XRaiseWindow(top_level->display, top_level->win);
	return 0;
}
